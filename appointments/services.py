...
def complete_appointment(appt, actor):
    ...
    # Đảm bảo luôn có hồ sơ khám trước khi hoàn tất ca.
    # Nếu chưa có, hiển thị thông báo rõ ràng cho bác sĩ.
    try:
    ...
    # Nếu không có bất kỳ đơn thuốc nào, chặn sớm với thông báo rõ ràng
    # thay vì để database trả về lỗi "Cannot create invoice: at least one prescription is required."
    mr = appt.medical_record
    if not mr.prescriptions.exists():
        raise ValueError("Vui lòng kê ít nhất một thuốc trước khi hoàn tất ca.")
